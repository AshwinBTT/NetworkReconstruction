...
5. Irreducibility code (Irreducible.c)
----------------------
...
6. input builder for CPLEX (weights_input.py)
----------------------------------------------
...
Output:
- a csv files of all the weights
...
9. About IPOPT + HSL MA97, and CPLEX
------------------------------------
- The IPOPT solver is freely available online: you can download its source or binaries from its official repository, and compile it on your system. For best performance and numerical stability we compile IPOPT with the HSL linear-solver MA97. If you wish to use MA97 you need an academic license from HSL.  
- The CPLEX solver comes via IBM ILOG CPLEX Optimization Studio. If you are a student or academic, you can apply for an academic license and download a full (unrestricted) version via IBM’s Academic/Institutional licensing program.  

To run our full pipeline, you will need:
1. IPOPT (optionally compiled with MA97, if you have the license)  
2. A licensed copy of CPLEX (academic or paid, depending on your eligibility)  

Once installed and licensed, you can compile and run the corresponding codes to build the final weighted network.

10. Factory-level extension (Factories.py)
------------------------------------------
This script is an optional post-processing step that takes the final *firm-level*
weighted network and allocates each firm-to-firm link across factories (production
units) using geographic distance. This is the “Network of Firms with Multiple
Production Units (Factories)” extension described in the paper.

Inputs:
- a factories CSV (one row per factory) with:
  - a firm identifier column (name containing "ciq_id" or "firm")
  - latitude column (name containing "lat")
  - longitude column (name containing "lon")
- weighted_network.txt
  Each line:  src_firm dst_firm weight

What it does:
- computes great-circle (Haversine) distances between factories,
- uses the exponential kernel  exp(-d/τ)  to prefer nearby factory pairs,
- computes within-firm factory prominence weights p_i(a),
- runs the two-step assignment procedure:
  Step 1: activate all factories (give each factory at least one outgoing link,
          unless the firm has zero outgoing links),
  Step 2: allocate any remaining firm-level links using prominence + distance.

What it preserves:
- no new firm pairs are created (factory links only appear where a firm-level
  link exists),
- firm self-links are not created by this step,
- total firm-to-firm weights are preserved exactly; when a firm has fewer
  outgoing links than factories, the corresponding firm-level link weight is
  split evenly across the factories that share it.

Output:
- output/factory_network_weighted.txt

Each line contains:
  src_firm dst_firm src_factory dst_factory src_lat src_lon dst_lat dst_lon weight

Here src_factory and dst_factory are 0-based factory indices, and lat/lon are
written in degrees.

Example runs:
    python3 Factories.py

    python3 Factories.py         --factories-csv <factories.csv>         --weighted-network <weighted_network.txt>         --output <factory_network_weighted.txt>         --tau-km 100         --seed 12345

The τ parameter controls how local the factory allocation is: smaller τ makes
links more local in space, while larger τ spreads links more broadly.
